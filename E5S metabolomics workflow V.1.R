...
HM_apply <- function(data) {
  result <- data
  hm <- as.numeric(apply(result, 2, function(x) min(x, na.rm = T)/2))
  final <- sapply(1:length(hm), function(y) replace(result[,y],which(is.na(result[,y])), min(result[,y], na.rm = T)/2))
  final2 <- as.data.frame(final)
  final2
}

dsr_hm <- data.frame(cbind(Label = dsr[,1], HM_apply(dsr[,-1]))) # exclude Label column
rownames(dsr_hm) <- rownames(dsr)
colnames(dsr_hm) <- colnames(dsr)
